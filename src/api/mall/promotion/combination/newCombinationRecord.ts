...
export const getCombinationRecordPage = async (params) => {
  return await request.get({ url: '/llm/promotion/combination-record/page', params })
}

// 查询一个拼团的完整拼团记录
export const getCombinationRecordPageByHeadId = async (params) => {
  return await request.get({ url: '/llm/promotion/combination-record/page-by-headId', params })
}

// 获得拼团记录的概要信息
export const getCombinationRecordSummary = async () => {
  return await request.get({ url: '/llm/promotion/combination-record/get-summary' })
}
